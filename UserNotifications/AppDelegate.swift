...
class AppDelegate: UIResponder, UIApplicationDelegate {
    ...
    func application(application: UIApplication, didFinishLaunchingWithOptions launchOptions: [NSObject: AnyObject]?) -> Bool {
        ...
        //app won't be dispalayed(action will perform in background)
        firstAction.activationMode = UIUserNotificationActivationMode.Background
        //button will have red background
        firstAction.destructive = true
        firstAction.authenticationRequired = false
        
        let secondAction:UIMutableUserNotificationAction = UIMutableUserNotificationAction()
        secondAction.identifier = "SECOND_ACTION"
        secondAction.title = "Second Act"
        ...
        //will ask for permission to inform about notification(i will only ask once during first start of app, then will save a choice in memory and even when app will be terminated it won't ask again)
